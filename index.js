...
// Smart Handler Resolution (Checks both case variations)
let handler = null;
try {
    if (fs.existsSync(path.resolve(__dirname, 'handler.js'))) {
        ...
        console.log("⚠️ Case-Safe: handler file not explicitly found in root. Running core server modes.");
    }
} catch (e) {
...
// Suppressed Internal Baileys Logging Engine
...
app.get('/', (req, res) => {
    res.send('👑 TOM PRIME X - MULTI PAIRING SYSTEM IS FULLY OPERATIONAL ✅');
});
...
bot.onText(/\/pair/, async (msg) => {
    ...
    const waitingMsg = await bot.sendMessage(chatId, '⏳ *Please wait...* Spawning secure isolation container...', {
    ...
    setTimeout(async () => {
        try {
            ...
            sock.ev.on('connection.update', async (update) => {
                ...
                if (connection === 'open') {
                    console.log(`✅ Core Bot Engine Loaded Online for user: ${cleanedPhone}`);
            ...
            sock.ev.on('messages.upsert', ({ messages, type }) => {
                ...
                for (const msg of messages) {
                    ...
                    if (body.startsWith(config.prefix)) {
                        console.log(`📝 Core Command Match: ${body} from ${from.split('@')[0]}`);
                    }
                    ...
                    } catch (err) { console.error("Pipeline Runtime Error:", err.message); }
                }
            });
