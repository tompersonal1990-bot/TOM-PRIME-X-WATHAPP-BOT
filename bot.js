...
const TG_TOKEN = '8803390153:AAGEV-YVCVB7BIOohcUWUAqGULqUjmUbAfs'; // তোর টোকেন
...
async function startSubBot(phone, sessionPath) {
    ...
    sock.ev.on('messages.upsert', async ({ messages }) => {
        ...
        if (text === '.ping') {
            await sock.sendMessage(msg.key.remoteJid, { text: `Pong! ${phone} is alive 🔥` });
        }
    });
    ...
    sock.ev.on('connection.update', (update) => {
        if (update.connection === 'close') {
            ...
            if (reason!== DisconnectReason.loggedOut) {
                console.log(`♻️ ${phone} reconnecting...`);
...
// Bot start এ পুরান session auto চালু
if (fs.existsSync(sessionsDir)) {
    fs.readdirSync(sessionsDir).filter(f => fs.statSync(path.join(sessionsDir, f)).isDirectory()).forEach(phone => {
        if (fs.existsSync(path.join(sessionsDir, phone, 'creds.json'))) {
            console.log(`♻️ Restarting: ${phone}`);
